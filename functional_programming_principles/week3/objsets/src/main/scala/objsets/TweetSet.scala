...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    val newthat = if (that.contains(elem)) that else that.incl(elem)
    left.union(right.union(newthat))
  }
  ...
  def mostRetweeted: Tweet = {
    ...
    if (left.isPresent && leftMost.retweets > elem.retweets)
      if (right.isPresent && rightMost.retweets > leftMost.retweets)
        rightMost
      else
        leftMost
    else if (right.isPresent && rightMost.retweets > elem.retweets)
      rightMost
    else
      elem
  }
