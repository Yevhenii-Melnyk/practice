...
import book.kotlin.util.toJava
...
class PongActorTest : SubjectSpek<ActorRef>({
    ...
    describe("Pong actor") {
        ...
        it("should respond with Pong") {
            println(subject)
            val future = subject.ask("Ping", 1000)
            assertEquals("Pong", future.toJava<String>().block(1000))
        }

        it("should fail on unknown message") {
            println(subject)
            val future = subject.ask("unknown", 1000)
            assertFailsWith(ExecutionException::class) {
                future.toJava<String>().block(1000)
            }
        }
