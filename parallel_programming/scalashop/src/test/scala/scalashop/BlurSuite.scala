...
class BlurSuite extends FunSuite {
  ...
  test("HorizontalBoxBlur.parBlur with radius 2 should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
    ...
    HorizontalBoxBlur.parBlur(src, dst, 2, 2)
  ...
  test("HorizontalBoxBlur.parBlur with 32 tasks") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
