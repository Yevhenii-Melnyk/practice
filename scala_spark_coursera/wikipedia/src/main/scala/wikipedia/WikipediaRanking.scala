...
object WikipediaRanking {
	...
	/** Returns the number of articles on which the language `lang` occurs.
		...
		*/
	def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)((agg, article) => {
		if (article.text.split(" ").contains(lang)) agg + 1
		else agg
	}, _ + _)
	 ...
	 */
	def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd.flatMap(article => {
		langs.filter(article.text.split(" ").contains(_))
			.map((_, article))
	}).groupByKey()
	...
	def timed[T](label: String, code: => T): T = {
		...
		timing.append(s"Processing $label took ${
			stop - start
		} ms.\n")
		result
	}
